...
public class AmazonClientNotificationQueue {
  ...
  public AmazonClientNotificationQueue() {
    AmazonCache.getInstance().set(notificationCache, cacheTime, new LinkedList<QueueElement>());
  }
  ...
  public void putNotification(IClientNotification notification, IClientNotificationFilter filter) {
    ...
    synchronized (m_queueLock) {
      LinkedList<QueueElement> m_queue = (LinkedList<QueueElement>) AmazonCache.getInstance().get(notificationCache);
