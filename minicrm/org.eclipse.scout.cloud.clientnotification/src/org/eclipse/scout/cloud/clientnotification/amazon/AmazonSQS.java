 ...
 ******************************************************************************/
package org.eclipse.scout.cloud.clientnotification.amazon;
...
public class AmazonSQS {
  ...
  private static final IScoutLogger LOG = ScoutLogManager.getLogger(AmazonSQS.class);
...
//      CreateQueueRequest createQueueRequest = new CreateQueueRequest().withQueueName("scoutcloud");
//      queueUrl = sqs.createQueue(createQueueRequest).getQueueUrl();
  ...
  private QueueElement getQueueElement(Message m) {
    try {
      LOG.info("Found Message" + m.getBody());
      return objectSerializer.deserialize(StringUtility.hexToBytes(m.getBody()), QueueElement.class);
    }
      ...
      LOG.warn("Unable to read Message: " + m.getBody());
    }
    return null;
  }
